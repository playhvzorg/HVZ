...
public class ImageService {
    ...
    public ImageService(IOptions<ImageServiceOptions> options)
    {
        ...
        uploadPath = options.Value.UploadPath;
    ...
    /// <summary>
    ///     Retreive the resource path for a specific User thumbnail of size specified by type <see cref="ImageSize" />. Does
    ...
    public string GetUserThumbnailResourceLink(string id, ImageSize imageSize)
    {
        return $"images/users/{id}_thumbnail_{(int)imageSize}.jpeg";
    }

    /// <summary>
    ///     Retreive the resource path for a specific Org thumbnail of size specified by type <see cref="ImageSize" />. Does
    ///     not check that the file exists.
    /// </summary>
    /// <param name="id">The Org ID associated with the thumbnail</param>
    /// <param name="imageSize">Desired image size</param>
    /// <returns>The path to the thumbnail web resource</returns>
    public string GetOrgThumbnailResourceLink(string id, ImageSize imageSize)
    {
        return $"images/orgs/{id}_thumbnail_{(int)imageSize}.jpeg";
    }
    ...
    {
        return File.Exists(Path.Combine(uploadPath, "users", $"{id}_thumbnail_64.jpeg")) &&
    ...
    {
        return File.Exists(Path.Combine(uploadPath, "orgs", $"{id}_thumbnail_64.jpeg")) &&
    ...
    /// <summary>
    ///     Write an uploaded file to the disk under the "user" sub folder along with small 64x64 px, medium 128x128 px, and
    ///     large 256x256 px thumbnail images
    /// </summary>
    ...
    /// <summary>
    ///     Write an uploaded file to the disk under the "org" sub folder along with small 64x64 px, medium 128x128 px, and
    ///     large 256x256 px thumbnail images
    /// </summary>
    /// <param name="file"></param>
    ...
    /// <summary>
    ///     Write an uploaded file to the disk and creates small 64x64 px, medium 128x128 px, and large 256x256 px thumbnail
    ///     files
    /// </summary>
    /// <param name="file"></param>
    ...
    public async Task SaveImage(IBrowserFile file, string fileName, string folder)
    {
        ...
        string[] fileContentType = file.ContentType.Split("/");
        if (fileContentType[0] != "image")
            throw new ArgumentException("File must be an image");

        string directoryPath = Path.Combine(uploadPath, folder);
        string filePath = Path.Combine(directoryPath, $"{fileName}.{fileContentType[1]}");
        await using var fs = new FileStream(filePath, FileMode.Create);
        using Stream imageStream = file.OpenReadStream(4096 * 4096 * 32);
    ...
    private SKBitmap CropSquare(SKBitmap src, ImageSize imageSize)
    {
        ...
        var scaled = new SKBitmap(size, size);
